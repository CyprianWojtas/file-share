...
export class FileDownload extends EventObject {
    constructor(path, connection, downloader, fileHandle, force = false) {
        super();
        this.path = path;
    ...
    static async _downloadEnqueued(fileToDownload) {
        let progress = 0;
        // @ts-ignore
...
export class DirectoryDownload extends EventObject {
    constructor(path, connection, downloader) {
        super();
        this.fileQueue = [];
        this.fileCount = 0;
    ...
    static async _downloadEnqueued(directoryToDownload) {
        ...
        const parseTree = async (dirHandle, dirInfo) => {
            for (const directory of dirInfo.directories || []) {
                ...
                const subdirInfo = await connection.getDirectory(directory.path);
